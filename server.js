...
// User registration - goes to pending list
...
// Login - checks if user is approved
...
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    const verification = await BinanceAPI.verifyApiKey(apiKey, secretKey, false);
    if (!verification.success) {
        return res.status(401).json({ success: false, message: 'Invalid Binance API keys. Enable Spot & Margin Trading.' });
    }
    const users = readUsers();
...
class BinanceAPI {
    ...
    static async verifyApiKey(apiKey, secret, useTestnet = false) {
        try {
            ...
            return { success: true, canTrade: data.canTrade };
...
app.post('/api/change-password', authenticate, async (req, res) => {
    if (!req.user.isOwner) {
        return res.status(403).json({ success: false, message: 'Only owner can change password' });
    }
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`New users must be approved by owner before using the bot.`);
});
